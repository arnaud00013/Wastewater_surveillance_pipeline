...
output_workspace <- "D:/Mirror/Covid19/Wastewater/Illumina/"
...
df_plot_mean_cov_per_sample_by_ORF$breadth_of_cov <- unname(vapply(X = 1:nrow(df_plot_mean_cov_per_sample_by_ORF),FUN = function(i) length(unique(subset(df_depth,(sample==df_plot_mean_cov_per_sample_by_ORF$Sample[i])&(ORF==df_plot_mean_cov_per_sample_by_ORF$ORF[i])&(depth>=100)&(!is.na(depth)))$position))/(v_orfs_length[df_plot_mean_cov_per_sample_by_ORF$ORF[i]]) ,FUN.VALUE = c(0.0)))
...
df_plot_time_series_S_mutation_of_interest_prevalence_per_location$nb_samples_in_which_SNV_is_detectable <- unname(vapply(X = 1:nrow(df_plot_time_series_S_mutation_of_interest_prevalence_per_location),FUN = function(i) length(unique(intersect(subset(df_depth,(position==get_position_from_mut_name(the_mut_name = df_plot_time_series_S_mutation_of_interest_prevalence_per_location$mutation_name[i]))&(depth>=100))$sample,names(table(names((v_samples_date[v_samples_date==df_plot_time_series_S_mutation_of_interest_prevalence_per_location$Sampling_date[i]]))))))),FUN.VALUE = c(0)))
...
if ((!"label_mut_ORF_effect"%in%colnames(df_prevalence_mut_of_interest_in_NCBI_WORLDWIDE_lineages))|(!"B.1.617.X"%in%df_prevalence_mut_of_interest_in_NCBI_WORLDWIDE_lineages$lineage)){
  ...
  df_sig_muts_prevalence_data_to_add <- read.csv2(file = paste0(output_workspace,"df_signature_muts_prevalence_VOCs_VUIs_as_of_2021_07_08.csv"),sep = ",",header = T,stringsAsFactors = FALSE)
...
get_lst_lineage_mutations_and_conf_score <- function(the_lineage, the_sample){
  ...
  the_coverage_adjusted_confidence_score <- sum(v_current_lineage_signature_muts%in%unique(subset(df_variants,(Sample==the_sample))$label_mut_in_marker_fmt))/sum(v_site_depth_genomic_positions_signature_muts>=100)
...
df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_prevalence$month_year <- format(as.Date(df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_prevalence$date,"%Y-%m-%d"),"%Y-%m")
df_results_method1 <- data.frame(month=v_lst_year_month,method="Lineage pre-selection based on the number of\nsignature mutations AND X is a prevalence matrix",nb_WW_samples=NA,nb_WW_detections=NA,WW_monthly_richness=NA,WW_monthly_Shannon_entropy=NA,WW_monthly_antilog_Shannon_entropy=NA,WW_monthly_Evenness=NA,stringsAsFactors = F)
for (i in 1:nrow(df_results_method1)){
  df_results_method1$nb_WW_samples[i] <- length(unique(subset(df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_prevalence,(!is.na(month_year))&(month_year==df_results_method1$month[i]))$Sample))
  df_results_method1$nb_WW_detections[i] <- sum((!is.na(df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_prevalence$month_year))&(df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_prevalence$month_year==df_results_method1$month[i]),na.rm=T)
  df_results_method1$WW_monthly_richness[i] <- length(unique((subset(df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_prevalence,(!is.na(month_year))&(month_year==df_results_method1$month[i])&(!is.na(lineage))))$lineage))
  df_results_method1$WW_monthly_Shannon_entropy[i] <- ifelse(test = df_results_method1$WW_monthly_richness[i]>0,yes=get_entropy(target = (subset(df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_prevalence,(!is.na(month_year))&(month_year==df_results_method1$month[i])))$lineage),no=NA)
  df_results_method1$WW_monthly_antilog_Shannon_entropy[i] <- 2^(ifelse(test = df_results_method1$WW_monthly_richness[i]>0,yes=df_results_method1$WW_monthly_Shannon_entropy[i],no=NA))
  df_results_method1$WW_monthly_Evenness[i] <- ifelse(test = df_results_method1$WW_monthly_richness[i]>0,yes=df_results_method1$WW_monthly_Shannon_entropy[i]/(log2(df_results_method1$WW_monthly_richness[i])),no=NA)
}

  #method2 : "Lineage pre-selection based on the number of\nsignature mutations AND X is a binary matrix"
df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_binary$month_year <- format(as.Date(df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_binary$date,"%Y-%m-%d"),"%Y-%m")
df_results_method2 <- data.frame(month=v_lst_year_month,method="Lineage pre-selection based on the number of\nsignature mutations AND X is a binary matrix",nb_WW_samples=NA,nb_WW_detections=NA,WW_monthly_richness=NA,WW_monthly_Shannon_entropy=NA,WW_monthly_antilog_Shannon_entropy=NA,WW_monthly_Evenness=NA,stringsAsFactors = F)
for (i in 1:nrow(df_results_method2)){
  df_results_method2$nb_WW_samples[i] <- length(unique(subset(df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_binary,(!is.na(month_year))&(month_year==df_results_method2$month[i]))$Sample))
  df_results_method2$nb_WW_detections[i] <- sum((!is.na(df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_binary$month_year))&(df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_binary$month_year==df_results_method2$month[i]),na.rm=T)
  df_results_method2$WW_monthly_richness[i] <- length(unique((subset(df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_binary,(!is.na(month_year))&(month_year==df_results_method2$month[i])&(!is.na(lineage))))$lineage))
  df_results_method2$WW_monthly_Shannon_entropy[i] <- ifelse(test = df_results_method2$WW_monthly_richness[i]>0,yes=get_entropy(target = (subset(df_sample_PANGO_lineages_frequencies_preselect_based_on_nb_sig_muts_and_X_is_binary,(!is.na(month_year))&(month_year==df_results_method2$month[i])))$lineage),no=NA)
  df_results_method2$WW_monthly_antilog_Shannon_entropy[i] <- 2^(ifelse(test = df_results_method2$WW_monthly_richness[i]>0,yes=df_results_method2$WW_monthly_Shannon_entropy[i],no=NA))
  df_results_method2$WW_monthly_Evenness[i] <- ifelse(test = df_results_method2$WW_monthly_richness[i]>0,yes=df_results_method2$WW_monthly_Shannon_entropy[i]/(log2(df_results_method2$WW_monthly_richness[i])),no=NA)
}

  #method3 : "Lineage pre-selection based on forward\nselection AND X is a binary matrix"
df_sample_PANGO_lineages_frequencies_preselect_based_on_forward_selection_and_X_is_binary$month_year <- format(as.Date(df_sample_PANGO_lineages_frequencies_preselect_based_on_forward_selection_and_X_is_binary$date,"%Y-%m-%d"),"%Y-%m")
df_results_method3 <- data.frame(month=v_lst_year_month,method="Lineage pre-selection based on forward\nselection AND X is a binary matrix",nb_WW_samples=NA,nb_WW_detections=NA,WW_monthly_richness=NA,WW_monthly_Shannon_entropy=NA,WW_monthly_antilog_Shannon_entropy=NA,WW_monthly_Evenness=NA,stringsAsFactors = F)
for (i in 1:nrow(df_results_method3)){
  df_results_method3$nb_WW_samples[i] <- length(unique(subset(df_sample_PANGO_lineages_frequencies_preselect_based_on_forward_selection_and_X_is_binary,(!is.na(month_year))&(month_year==df_results_method3$month[i]))$Sample))
  df_results_method3$nb_WW_detections[i] <- sum((!is.na(df_sample_PANGO_lineages_frequencies_preselect_based_on_forward_selection_and_X_is_binary$month_year))&(df_sample_PANGO_lineages_frequencies_preselect_based_on_forward_selection_and_X_is_binary$month_year==df_results_method3$month[i]),na.rm=T)
  df_results_method3$WW_monthly_richness[i] <- length(unique((subset(df_sample_PANGO_lineages_frequencies_preselect_based_on_forward_selection_and_X_is_binary,(!is.na(month_year))&(month_year==df_results_method3$month[i])&(!is.na(lineage))))$lineage))
  df_results_method3$WW_monthly_Shannon_entropy[i] <- ifelse(test = df_results_method3$WW_monthly_richness[i]>0,yes=get_entropy(target = (subset(df_sample_PANGO_lineages_frequencies_preselect_based_on_forward_selection_and_X_is_binary,(!is.na(month_year))&(month_year==df_results_method3$month[i])))$lineage),no=NA)
...
Richness_over_Seqrate_across_methods_gg <- ggplot(data = df_results_methods,aes(x=as.factor(as.character(method)),y = WW_monthly_richness/nb_WW_samples,fill=as.factor(as.character(method)))) + geom_violin() + geom_boxplot(width=0.075,fill="white") + geom_jitter() + xlab("method") + ylab("Monthly richness/\nNumber of sequenced samples per month") + theme_bw() + theme(axis.title = element_text(size=12),axis.text = element_text(size=12),legend.position = "right",axis.text.x=element_blank()) +
  stat_compare_means(method = "wilcox",comparisons = list(c("Lineage pre-selection based on forward\nselection AND X is a binary matrix","Lineage pre-selection based on the number of\nsignature mutations AND X is a binary matrix"),c("Lineage pre-selection based on forward\nselection AND X is a binary matrix","Lineage pre-selection based on the number of\nsignature mutations AND X is a prevalence matrix"),c("Lineage pre-selection based on the number of\nsignature mutations AND X is a binary matrix","Lineage pre-selection based on the number of\nsignature mutations AND X is a prevalence matrix"))) + labs(fill="method")

Shannon_Entropy_over_Seqrate_across_methods_gg <- ggplot(data = df_results_methods,aes(x=as.factor(as.character(method)),y = WW_monthly_Shannon_entropy/nb_WW_samples,fill=as.factor(as.character(method)))) + geom_violin() + geom_boxplot(width=0.075,fill="white") + geom_jitter() + xlab("method") + ylab("Monthly Shannon Entropy/\nNumber of sequenced samples per month") + theme_bw() + theme(axis.title = element_text(size=12),axis.text = element_text(size=12),legend.position = "right",axis.text.x=element_blank()) +
  stat_compare_means(method = "wilcox",comparisons = list(c("Lineage pre-selection based on forward\nselection AND X is a binary matrix","Lineage pre-selection based on the number of\nsignature mutations AND X is a binary matrix"),c("Lineage pre-selection based on forward\nselection AND X is a binary matrix","Lineage pre-selection based on the number of\nsignature mutations AND X is a prevalence matrix"),c("Lineage pre-selection based on the number of\nsignature mutations AND X is a binary matrix","Lineage pre-selection based on the number of\nsignature mutations AND X is a prevalence matrix"))) + labs(fill="method")

Evenness_over_Seqrate_across_methods_gg <- ggplot(data = df_results_methods,aes(x=as.factor(as.character(method)),y = WW_monthly_Evenness/nb_WW_samples,fill=as.factor(as.character(method)))) + geom_violin() + geom_boxplot(width=0.075,fill="white") + geom_jitter() + xlab("method") + ylab("Monthly Evenness/\nNumber of sequenced samples per month") + theme_bw() + theme(axis.title = element_text(size=12),axis.text = element_text(size=12),legend.position = "right",axis.text.x=element_blank()) +
  stat_compare_means(method = "wilcox",comparisons = list(c("Lineage pre-selection based on forward\nselection AND X is a binary matrix","Lineage pre-selection based on the number of\nsignature mutations AND X is a binary matrix"),c("Lineage pre-selection based on forward\nselection AND X is a binary matrix","Lineage pre-selection based on the number of\nsignature mutations AND X is a prevalence matrix"),c("Lineage pre-selection based on the number of\nsignature mutations AND X is a binary matrix","Lineage pre-selection based on the number of\nsignature mutations AND X is a prevalence matrix"))) + labs(fill="method")
