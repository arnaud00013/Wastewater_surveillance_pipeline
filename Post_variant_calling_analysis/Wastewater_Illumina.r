...
#if there are new data to add or if prevalence dataframe is incomplete, add the appropriate information
...
ggplot(data = df_plot_confidence_scores_PANGO_lineages_of_interest_per_location,mapping=aes(x=date,y=coverage_adjusted_confidence_score)) + geom_col(mapping=aes(fill=factor(label_lineage,levels=v_lineages_of_interest_with_who_desgnation[names(sort(table(subset(df_detected_marker_mutations_in_ww_samples,PANGO_lineage%in%v_lineages_of_interest)$PANGO_lineage),decreasing = F))])),position = position_dodge(width = 0.9)) +
  xlab("Sampling date") + ylab(paste0("Confidence score\n (proportion of detectable signature mutations that are detected)")) + theme_bw() + theme(axis.title = element_text(size=12),axis.text = element_text(size=12),legend.title = element_text(size=12),legend.text = element_text(size=8),axis.text.x = element_text(size=9, angle = 60,hjust=1))+ scale_y_continuous(limit=c(0,1),breaks = seq(0,1.01,0.1)) + facet_wrap(~location, ncol=1) + labs(fill="VOCs and VOIs in increasing\norder of prevalence") + scale_fill_manual(values=palette_PANGO_lineages_of_interest_with_who_designation)
...
ggplot(data = df_plot_confidence_scores_PANGO_lineages_of_interest_per_location,mapping=aes(x=date,y=nb_signature_mutations)) + geom_col(mapping=aes(fill=factor(label_lineage,levels=v_lineages_of_interest_with_who_desgnation[names(sort(table(subset(df_detected_marker_mutations_in_ww_samples,PANGO_lineage%in%v_lineages_of_interest)$PANGO_lineage),decreasing = F))])),position = position_dodge(width = 0.9)) +
  xlab("Sampling date") + ylab(paste0("Number of detected signature mutations")) + theme_bw() + theme(axis.title = element_text(size=12),axis.text = element_text(size=12),legend.title = element_text(size=12),legend.text = element_text(size=8),axis.text.x = element_text(size=9, angle = 60,hjust=1))+ scale_y_continuous(limit=c(0,max(df_plot_confidence_scores_PANGO_lineages_of_interest_per_location$nb_signature_mutations)+3),breaks = seq(0,max(df_plot_confidence_scores_PANGO_lineages_of_interest_per_location$nb_signature_mutations)+3,3)) + facet_wrap(~location, ncol=1) + labs(fill="VOCs and VOIs in increasing\norder of prevalence") + scale_fill_manual(values=palette_PANGO_lineages_of_interest_with_who_designation) + geom_hline(yintercept = 3,lty=2,col="black")
...
#save checkpoint
library("session")
save.session(file = paste0(output_workspace,"Checkpoint_",gsub(pattern = ":",replacement = "_",x = gsub(pattern = " ",replacement = "_",x = date())),"_RSession.Rda"))
...
the_f_parallel_preselect_based_on_nb_sig_muts_and_X_is_prevalence <- function(i_cl){
  ...
  for (z in the_vec){
    ...
    print("df_variants is accessible!")
